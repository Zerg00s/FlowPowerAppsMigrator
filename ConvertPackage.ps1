...
$CurrentPath = (Get-Location).Path + "\PowerAppsConverter\"

# Clean previously generated packages
Get-ChildItem $CurrentPath -Filter "Converted*" -Recurse| Remove-Item

$packages = Get-ChildItem -Path .\PowerAppsConverter\src -Filter "*.zip"
...
for($k=0;$k -lt $packages.count; $k++){
    ...
    if ($msappPackages -ne $null -and $msappPackages.Count -ne 0){
        for($y=0;$y -lt $msappPackages.count; $y++){
            $msAppPackage = $msappPackages[$y];
            $msappPackagename = $msAppPackage.Name
